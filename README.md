...
v1.2では、運営者向けの管理画面を本格化し、チェーンの追加・有効/無効切り替えをブラウザ上でできるようにしました。

- 管理画面は `?admin=1` を付けたときだけ表示されます
- 管理画面内に「チェーン管理」セクションを追加しました
- 現在登録されているチェーン一覧、チェーン名、ジャンル、公式サイトURL、キャンペーン一覧URL、有効/無効を管理できます
- 「チェーン追加」フォームから新しいチェーンを追加できます
- 追加したチェーンは `localStorage` に保存され、ページ再読み込み後も残ります
- OFF にしたチェーンは管理画面では残しますが、検索・おすすめ表示の対象外になります
- 次の段階では、この一覧を `chains.json` や Apps Script に反映する運用に進める構成です

## チェーンを追加する方法

v1.1以降は、チェーン情報をコードに直接書かず、[chains.json](chains.json) に1件追加するだけで管理できます。

1. [chains.json](chains.json) を開く
2. 1件のチェーンオブジェクトを追加する
...
chains.json  # チェーンマスター。チェーンを追加するときはここを編集する
...
const CHAINS_JSON_URL = 'https://raw.githubusercontent.com/tkvforeverkter-cyber/today-meal-deal-navi/main/chains.json';
...
function updateCampaignSources() {
  const response = UrlFetchApp.fetch(CHAINS_JSON_URL);
  const chains = JSON.parse(response.getContentText());
  const enabledChains = (Array.isArray(chains) ? chains : []).filter((chain) => chain.enabled !== false);

  enabledChains.forEach((chain) => {
    Logger.log('巡回対象:', chain.name);
    UrlFetchApp.fetch(chain.campaignUrl || chain.officialSite || 'https://example.com');
  });

  return enabledChains.map((chain) => ({
    id: chain.id,
    name: chain.name,
    campaignUrl: chain.campaignUrl,
    officialSite: chain.officialSite,
  }));
}
